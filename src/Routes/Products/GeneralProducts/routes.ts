...
productsRoutes.route("/products").post(MiddlewareAuthorization.authorization, MiddlewareAuthorization.isAdmin, upload.fields([{name:"ProductImages", maxCount: 10}]),multerErrorHandler,validateUploadedImageContent,(req: Request, res: Response) => {RegisterProductsController.register(req, res)})
...
productsRoutes.route("/products/:id_product").patch(MiddlewareAuthorization.authorization, MiddlewareAuthorization.isAdmin,upload.fields([{name:"ProductImages", maxCount: 10}]),multerErrorHandler,validateUploadedImageContent,(req: Request, res: Response) =>{EditProductDatasController.edit(req, res)})
